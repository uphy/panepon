...
- [ ] 横持ちのスマホ専用レイアウト（保留）
  - 今は 800×520 のデスクトップ用レイアウトを縮小して出すので小さい（Pixel 7 横持ちでマスが実画面 25px）
  - 盤面を高さいっぱいにしても 12 段で 412px なのでマスは最大 34px。指で押す目安の 36px に届かないため、横持ちは縦持ちより必ず劣る。作るなら得点・残り時間・せり上げの矢印を盤面の左右へ寄せる BoardView の変更が要る
- [ ] オンライン対戦（未着手。サーバーが要る）
  - 2P VERSUS は同じ画面を2人で触る前提で、スマホでは成立しにくい。決定論的シミュレーションなので seed と入力列を送り合うロックステップ方式で作れる。中継は Cloudflare Durable Objects の WebSocket が配信先と同じ場所で済む
  - 非同期の「同じ seed でスコア勝負」なら、サーバーはスコアを預かるだけで済む。先にこちらを作るほうが小さい
