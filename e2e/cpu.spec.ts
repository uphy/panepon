...
test("CPU 対戦: 2P 側を CPU が動かし、放置すると負けて勝敗が記録される", async ({ page }) => {
  ...
  const cleared = await page.evaluate(() => (window as any).__panepon.game.boards[1].panelsCleared);
  ...
  const winner = await page.evaluate(() => (window as any).__panepon.game.winner);
  expect(winner).toBe(1);
  const stored = await page.evaluate(() => JSON.parse(localStorage.getItem("panepon.highscores.v1") ?? "{}"));
...
test("ハイスコア: エンドレスの結果が保存され、メニューに表示される", async ({ page }) => {
  ...
  await page.waitForFunction(() => Boolean((window as any).__panepon?.game) && !(window as any).__panepon.game.finished);
  await page.evaluate(() => {
    const b = (window as any).__panepon.game.boards[0];
    b.score = 1000;
    const col: number[] = [];
  ...
  await page.waitForFunction(() => (window as any).__panepon.game.finished);
  await page.waitForTimeout(200);
  const after = await page.evaluate(() => JSON.parse(localStorage.getItem("panepon.highscores.v1") ?? "{}"));
