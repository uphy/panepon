...
export class Button extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, text: string, onPress: () => void, opts: ButtonOptions = {}) {
    ...
    this.label = scene.add
      .text(0, 0, text, { fontFamily: FONT, fontSize: `${fontSize}px`, color: opts.color ?? TEXT_COLOR })
      .setOrigin(0.5);
    ...
    this.on("pointerover", () => this.bg.setFillStyle(0x3e3e56));
    this.on("pointerout", () => this.bg.setFillStyle(this.baseColor));
    this.on("pointerdown", (_p?: Phaser.Input.Pointer, _x?: number, _y?: number, event?: Phaser.Types.Input.EventData) => {
      ...
      onPress();
      this.scene.time.delayedCall(120, () => this.bg.setFillStyle(this.baseColor));
    });
